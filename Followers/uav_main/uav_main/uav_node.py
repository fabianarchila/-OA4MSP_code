...
class Node1(Node):
    ...
    def __init__(self):
        ...
        self.msg_temp.data = "-----Initializing drone----"
    ...
    def timer_callback(self):
        self.get_logger().info(" Battery: %s" % self.vehicle.battery)
    ...
    def action_selection(self, msg: String):
        if msg.data =="enable_connection" and self.conn_state== False:
            ...
            try:
                ...
                self.get_logger().info(" Global Location: %s" % self.vehicle.location.global_frame)
                self.get_logger().info("Device conected")
                self.msg_temp.data = "Device conected"
                self.logs_out.publish(self.msg_temp)
                self.conn_state=True
            except:
                self.msg_temp.data = "Connection not available"
                self.logs_out.publish(self.msg_temp)
            
        elif msg.data == "close_connection"and self.conn_state==True:
            self.vehicle.close()
            self.get_logger().info("Connection finished")
            self.msg_temp.data = "Connection finished"
            self.logs_out.publish(self.msg_temp)
            self.conn_state=False
            ...
            self.msg_temp.data = "Takeoff starting"
            self.logs_out.publish(self.msg_temp)
            if self.vehicle.is_armable == True:
                self.vehicle.armed = True
                self.msg_temp.data = " Waiting for arming..."
                now=time.time()
                ...
                if self.vehicle.armed ==True:
                    self.vehicle.mode = VehicleMode("GUIDED")
                    altitude = 3
                    self.msg_temp.data = "Taking off!"
                    self.logs_out.publish(self.msg_temp)
                    self.vehicle.simple_takeoff(altitude)  # Take off to target altitude
                    now=time.time()
                    while True and time.time()-now<5:
                        self.msg_temp.data = "Altitude: {}".format(self.vehicle.location.global_relative_frame.alt)
                        self.logs_out.publish(self.msg_temp)
                        # Break and return from function just below target altitude.
                        if self.vehicle.location.global_relative_frame.alt >= altitude * 0.95:
                            self.msg_temp.data = "Reached target altitude"
                            self.logs_out.publish(self.msg_temp)
                            break
                        time.sleep(1)

                else:
                    self.msg_temp.data = " Not armed"
                    self.logs_out.publish(self.msg_temp)

            else:
                self.msg_temp.data = "Not armable"
                self.logs_out.publish(self.msg_temp)
        elif msg.data == "start_mission" and self.conn_state == True:
            if self.go_to_state == False:
                self.msg_temp.data = " Starting following"
                self.logs_out.publish(self.msg_temp)
                self.vehicle.mode = VehicleMode("GUIDED")
            ...
            self.msg_temp.data = "posodro:lat={}, lon={},alt={}, n_data={}".format(self.latitude,self.longitude,self.altitude,self.data_recieved)
            self.logs_out.publish(self.msg_temp)
